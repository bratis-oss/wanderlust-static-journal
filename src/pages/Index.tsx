...
import { getFeaturedPosts, getLatestPosts, getUniqueDestinations, getPostsByCategory } from '../data/posts';
...
const Index = () => {
  ...
  // Obtener posts de escapadas en lugar de destinos populares
  const escapadasPosts = getPostsByCategory('escapadas');
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow">
        ...
        {/* Escapadas Section - Reemplazando Featured Posts */}
        <section className="container mx-auto px-4 py-16">
          <h2 className="text-3xl font-serif font-bold text-foreground mb-10 text-center">
            {translate("weekend-getaways")}
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {escapadasPosts.slice(0, 3).map(post => (
          ...
          <div className="text-center mt-8">
            <Link 
              to="/getaways" 
              className="inline-block bg-primary text-white hover:bg-primary/90 font-medium px-6 py-2 rounded-full transition-all transform hover:scale-105"
            >
              {translate("view-all-getaways")}
            </Link>
          </div>
        </section>
